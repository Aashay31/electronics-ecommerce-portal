...
function ReviewForm({ initialReview, onSubmit, onCancel, isSubmitting }) {
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <input
            ...
            className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
            maxLength={120}
            required
          />
        </div>
        <div>
          <label className="text-sm font-semibold text-slate-700">Quick tips</label>
          <div className="mt-2 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-3 text-xs text-slate-500">
      ...
      <div>
        <label className="text-sm font-semibold text-slate-700">Review details</label>
        <textarea
          value={comment}
          onChange={(event) => setComment(event.target.value)}
          placeholder="Tell us about quality, usability, and performance."
          rows={4}
          className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200"
          maxLength={2000}
          required
        />
      </div>
