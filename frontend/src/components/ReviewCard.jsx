...
function ReviewCard({ review, isOwner, onHelpful, onEdit, onDelete, isBusy }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white dark:bg-slate-900 p-6 shadow-sm transition hover:shadow-md">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 text-sm font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500">
            {initials}
          </div>
          <div>
      ...
      <div className="mt-4 space-y-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-400 dark:text-slate-500">
            {visibleComment}
          </p>
          {isLong && (
            <button
          ...
          {isOwner && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full border border-slate-200 dark:border-white/10 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-indigo-200 hover:text-indigo-600"
              >
