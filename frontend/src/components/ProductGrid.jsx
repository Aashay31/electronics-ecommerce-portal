...
function ProductGrid({ products, isLoading }) {
    ...
    return (
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {Array.from({ length: 8 }).map((_, index) => (
          <div
            key={`skeleton-${index}`}
            className="h-80 rounded-2xl border border-slate-100 bg-white p-4 shadow-md"
          >
            ...
            <div className="mt-6 h-9 w-full animate-pulse rounded-full bg-slate-100" />
          </div>
        ))}
      </div>
    );
  ...
  if (!products.length) {
    return (
      <div className="flex min-h-[240px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white p-10 text-center">
