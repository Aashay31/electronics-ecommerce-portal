    ...
    <div className="flex min-h-screen flex-col bg-slate-50 dark:bg-slate-900 dark:bg-slate-950">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 pt-36 pb-8 sm:px-6 lg:px-8 md:pt-28">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <Link
            to="/orders"
            className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 dark:text-slate-400 transition hover:text-slate-900"
          >
          ...
          {order.orderStatus !== "Cancelled" && (canCancel ? (
            ...
            <div className="rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900 px-4 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400" title={cancelMessage}>
              {cancelMessage}
            </div>
          ))}
        </div>
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
          {/* Header */}
          <div className="border-b border-slate-100 dark:border-white/10 bg-slate-900 p-6 sm:p-8">
          ...
          <div className="border-b border-slate-100 p-6 sm:p-8">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Order Status</h2>
            ...
            {isCancelled ? (
              ...
              <div className="relative mt-8">
                <div className="absolute left-0 top-1/2 hidden h-1 w-full -translate-y-1/2 bg-slate-100 dark:bg-slate-800 sm:block" />
                <div 
          ...
          <div className="grid gap-0 divide-y divide-slate-100 dark:divide-white/5 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
            {/* Shipping Info */}
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-400">
                <div className="flex justify-between">
                  <span>Method</span>
                  <span className="font-semibold text-slate-900 dark:text-white">{order.paymentMethod}</span>
                </div>
                <div className="flex justify-between">
                  <span>Status</span>
                  <span className="font-semibold text-slate-900 dark:text-white">{paymentLabel}</span>
                </div>
                {order.razorpayPaymentId && (
                  <div className="flex justify-between">
                    <span>Payment ID</span>
                    <span className="font-semibold text-slate-900 dark:text-white">{order.razorpayPaymentId}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span className="font-semibold text-slate-900 dark:text-white">₹{order.totalAmount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Shipping</span>
          ...
          {/* Items */}
          <div className="border-t border-slate-100 bg-slate-50 dark:bg-slate-900 dark:border-white/10 dark:bg-slate-800 p-6 sm:p-8">
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Ordered Items</h2>
            <div className="mt-6 space-y-4">
              {order.items.map((item) => (
                <div key={item.product._id} className="flex flex-col gap-4 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-950 p-4 shadow-sm sm:flex-row sm:items-center">
                  <div className="h-24 w-24 shrink-0 rounded-xl bg-slate-50 dark:bg-slate-900 p-2">
                    <img 
                  ...
                  <div className="flex-1">
                    <Link to={`/products/${item.product._id}`} className="font-semibold text-slate-900 dark:text-white hover:text-indigo-600 hover:underline">
                      {item.product.productName}
                    </Link>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{item.product.category}</p>
                    <div className="mt-2 flex items-center justify-between">
                      <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Qty: {item.quantity}</p>
